...
- `NoOcclusion`: 2枚のDisplayがほぼ重ならない。座標変換・クリック・スクロール確認用。
- `PartialOcclusion`: 2枚のDisplayが一部重なる。開発・練習・調整用。
...
| クリック | Aボタン | focus中Displayのcursor位置にあるUI要素をクリックする |
